...
> Score after building (Phase 4 of the [guideline](02-development-guideline.md)). Re-score after any element change.
...
| 9 | Skills Layer | Each skill invoked as a unit produces its contracted output; failure modes behave as specified. Library-sourced skills: no leftover `{{…}}` placeholders, `description` trigger rewritten for this agent | |
...
**Regression rule:** any case flipping **1 → 0** after a change → back to Phase 2 for the owning element, even if the total still passes.

## On failure

Any 0 on a required element → return to that element in the spec (Phase 2), fix, rebuild, **re-score the whole checklist** — fixes regress neighbors more often than expected. After any fix, re-run the **whole eval set**, not just the failed case — that is what catches regressions.
