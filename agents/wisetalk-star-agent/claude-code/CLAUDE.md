# WiseTalk STAR Agent Project

Agent 1 of the WiseTalk AI Communication Coach system: the **STAR Interviewer**. The `wisetalk-star-agent` sub-agent coaches a user through the STAR behavioural-story model — forces the 4 mandatory fill-in cards (Situation · Task · Action · Result), generates a polished interview-ready narrative, runs the iterative critique loop, and delivers the final text (generated from `../agent-spec.md` using the [Claude Code mapping](../../../templates/03-claude-code-mapping.md)).

One of 8 individual Expert Agents (STAR / SCRTV / MECE / PREP / SCQA / RIDE / FFC / Funnel) — each with its model hardcoded, dispatched by the Router Agent (`wisetalk-router-agent`). This agent serves **STAR only**.
...
```
config/model-reference.md       — STAR reference (extracted from the shared reference/wisetalk-model-catalog.md; agents read-only)
...
Invoke the agent with a routed request: *"use_case: Job_Interview — I have an interview tomorrow for a project manager role. They'll ask 'tell me about a time you handled a conflict.'"*
It forces the 4 STAR cards, generates the narrative, critiques it in 3 points, iterates with the user, and delivers the final text with the disclaimer.
