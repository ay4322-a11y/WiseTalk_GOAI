...
Hill-climbing eval set for the Funnel Refiner — Agent of the 8 WiseTalk Expert Communication Agents. Each spec change re-runs this set; a case flipping PASS -> FAIL goes back to the owning element.
...
| # | Case | run-1 |
|---|------|:-----:|
| 1 | Task_Delegation: 500-word vendor email (mandatory scenario) | — |
| 2 | Complex_Instruction: 400-word onboarding brief (mandatory scenario) | — |
| 3 | Short text: the gate fires (mandatory scenario) | — |
| 4 | Revision request: restore the deadline (success criterion) | — |
| 5 | Delivery integrity (success criterion) | — |
| 6 | Out-of-model referral (model-boundary) | — |
| 7 | Seeded defect (acceptance-check adversarial seed) | — |
| 8 | Battle Arena on the delivered summary (Battle Arena wiring) | — |
| 9 | Hallucination check on the accepted summary (hallucination-check wiring) | — |
| 10 | Input gate on the OriginalText card (pre-output position) | — |
| 11 | Output gate BLOCK → re-compression (validity gating) | — |
| 12 | Retry exhaustion → WARN delivery (retry cap) | — |
| 13 | Gate PASS (clean summary) | — |
| | **Total** | **0 / 13 scored** |
