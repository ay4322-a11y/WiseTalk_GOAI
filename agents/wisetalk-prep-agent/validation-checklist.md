# Validation Checklist — wisetalk-prep-agent (PREP Speaker)

> Filled-in example of [templates/04-validation-checklist.md](../../templates/04-validation-checklist.md) for the PREP Speaker — Agent 4 of the 8 WiseTalk Expert Communication Agents. Scored against the [agent spec](agent-spec.md); eval cases live in [evals/eval-cases.md](claude-code/evals/eval-cases.md).
...
- [x] Agent body frontmatter: `name: wisetalk-prep-agent`, `model: sonnet`, `tools: Read, Glob, Grep, Write`
- [x] Description frontmatter is a routing target: names the model ("PREP Speaker (Agent 4)"), names its 4 use cases, states "Do NOT use for other communication models", "Do NOT use for routing/classifying"
- [x] Model is baked in: agent body contains `## Model reference — PREP (baked in; no catalog read)` — no runtime catalog read anywhere
...
- [x] E1 Task Input: use_case + situation from Router; PREP fields; revision requests; empty-input and out-of-model handling
- [x] E2 Context Builder: PREP expert role; P→R→E→P structure; 4 fill-in fields; generation prompt; critique dimensions; baseline §1
...
- [x] E13 Reflection: 5-item self-check; hill-climbing eval loop (24/26 baseline); regression rule
...
|---|------|-------------|--------|
| 1 | Elevator_Pitch: user wants a 30-second pitch for automating the monthly sales report | Agent force-fills the 4 PREP cards (Point · Reason · Example · Action), generates a tight spoken answer, critiques it, iterates to acceptance | ✅ PASS |
| 2 | Public_Comment: user wants a short public comment supporting a team decision | Same loop for a different PREP use case; model integrity P→R→E→P holds | ✅ PASS |
| 3 | Quick_Meeting_Speech: user drafts a 1-minute speech for a stakeholder meeting | Cards partially filled — agent asks for the missing ones before generating (Skill-3 gate) | ✅ PASS |
| 4 | Revision: "make it shorter — it needs to be spoken aloud in under 30 seconds" | Draft rewritten via Skill-7 with the revision visibly applied; iteration counter incremented | ✅ PASS |
...
**Score: 26/26** (baseline: 24/26 — Elements 7 and 12 seeded at 1 in the archetype's first run; this agent's eval set re-scored at 26/26, the hill-climbing baseline for the PREP agent).
