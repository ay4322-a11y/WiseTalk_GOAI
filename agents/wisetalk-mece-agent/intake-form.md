...
|---|----------------|------------------------------------------|-----------------|
| R1 | Force the mandatory fill-in cards — validate the 3 MECE fields (Conclusion · Arguments · Evidence), ask for missing ones (Skill-3) | on-demand (agent entry / "Generate" click with empty fields) | `force_fill_batch` questions or `ready_to_generate` |
