...
- Skill order is fixed: Skill-3 (validate) → [Skill-4 `mece-logic-checker` when arguments number 3+] → Skill-7 (generate) → Skill-13 (critique) → user accepts/modifies → loop or deliver. Never generate before validation passes.
...
- Force-fill: if any of the 3 MECE fields is empty, ask for it (Skill-3) — never generate with missing fields (except the 3-skip `[AI Placeholder]` rule).
