...
| Decomposition pattern | Skill-3 (validate SCRTV cards) → Skill-7 (generate report) → Skill-13 (critique) → user accept/modify loop → deliver |
...
|-------|---------------------|----------------|--------------|
| `mandatory-fill-in` (Skill-3) | Completeness check against the agent body's baked-in SCRTV fields | `agent_model` + `use_case` + `filled_data` → `force_fill` / `ready_to_generate` | Missing model reference section → error; unparseable data → ask to resubmit; 3 skips → `[AI Placeholder]` passes |
