# Agent Validation Checklist — wisetalk-ride-agent

**Agent:** `wisetalk-ride-agent` (Agent 6 — RIDE Negotiator) · **Tier:** Standard · **Date:** 2026-08-09 · **Scorer:** template pack / manual static + behavioral run
...
| 12 | Observation Feedback | Large results summarized with provenance kept; a tool error is visibly handled in the loop; a claim can be walked back to its source (critique → RIDE dimension) | 1 |
| 13 | Reflection & Optimization | A seeded defect (a draft missing the Risk section) is caught by the critique and re-executed; the 3-iteration cap is respected; the acceptance signal is observed in a real run | 2 |
...
|---|--------------|----------------------|:----------------:|
| 1 | `use_case: Salary_Negotiation` — "My boss rejected my budget proposal because he thinks it's too high. How can I convince him?" (no cards filled) | Skill-3 returns `force_fill_batch` listing ALL 4 RIDE fields at once (Risk · Interest · Difference · Effect), not a draft | ✅ 1 |
| 2 | Same case, cards filled (Risk/Interest/Difference/Effect) | Skill-3 returns `ready_to_generate`; Skill-7 draft follows R→I→D→E with every card value present | ✅ 1 |
| 3 | Draft from case 2 → Skill-13 | Exactly 3 critique points (model integrity · tone · logic); no rewrite inside the critique | ✅ 1 |
| 4 | User replies "modify it — make the tone less aggressive" × 3 | Each iteration increments; at iteration 3 the loop force-exits with the best draft — never continues | ✅ 1 |
...
| 6 | User asks for STAR interview coaching ("can you coach me for my interview instead?") | Agent refers the request back to the Router Agent — does NOT switch models (spec Element 4 N/A behavior) | ✅ 1 |
