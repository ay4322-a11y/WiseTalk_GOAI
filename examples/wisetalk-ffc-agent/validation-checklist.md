# Validation Checklist — wisetalk-ffc-agent (FFC Master)

> Filled-in example of [templates/04-validation-checklist.md](../../templates/04-validation-checklist.md) for the FFC Master — Agent 7 of the 8 WiseTalk Expert Communication Agents. Scored against the [agent spec](agent-spec.md); eval cases live in [evals/eval-cases.md](claude-code/evals/eval-cases.md).
...
- [x] Agent body frontmatter: `name: wisetalk-ffc-agent`, `model: sonnet`, `tools: Read, Glob, Grep, Write`
- [x] Description frontmatter is a routing target: names the model ("FFC Master (Agent 7)"), names its 4 use cases, states "Do NOT use for other communication models", "Do NOT use for routing/classifying"
- [x] Model is baked in: agent body contains `## Model reference — FFC (baked in; no catalog read)` — no runtime catalog read anywhere
...
- [x] E1 Task Input: use_case + situation from Router; FFC fields; revision requests; empty-input and out-of-model handling
- [x] E2 Context Builder: FFC expert role; F→F→C structure; 3 fill-in fields; generation prompt; critique dimensions; baseline §1
...
|---|------|-------------|--------|
| 1 | Team_Recognition: recognise a team member who turned a difficult client meeting around | Agent force-fills the 3 FFC cards (Feeling · Fact · Compare), generates warm behaviour-based recognition, critiques it, iterates to acceptance | ✅ PASS |
| 2 | Peer_Feedback: give a colleague specific praise after a release | Same loop for a different FFC use case; Fact specific and observable (critique dim 1) | ✅ PASS |
| 3 | Ice_Breaking: cards partially filled — Compare missing | Agent asks for the missing card before generating (Skill-3 gate) | ✅ PASS |
| 4 | Revision: "add the business impact — it's for the monthly newsletter" | Draft rewritten via Skill-7 with the revision visibly applied; iteration counter incremented | ✅ PASS |
...
**Score: 26/26** (baseline: 24/26 — Elements 7 and 12 seeded at 1 in the archetype's first run; this agent's eval set re-scored at 26/26, the hill-climbing baseline for the FFC agent).
