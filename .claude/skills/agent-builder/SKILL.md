...
- Every generated agent inherits the all-agents baseline in `reference/behavioral-guidelines.md`: copy it to the target project as `docs/behavioral-guidelines.md`, reference it from the generated `CLAUDE.md` and the agent body's Rules section (per the mapping's skeleton), and record deviations — if any — in spec Element 2.
